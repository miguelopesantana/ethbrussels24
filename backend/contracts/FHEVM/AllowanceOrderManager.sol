...
interface IDarkPool {
    function depositDP(uint8 tokenId, uint32 amount) external;
}

contract EncryptedERC20 is BridgeContract, Ownable2Step {
    event Transfer(address indexed from, address indexed to);
    event Approval(address indexed owner, address indexed spender);
    event Mint(address indexed to, uint64 amount);

    uint64 private _totalSupply;
    string private _name = "Encrypted USDC";
    string private _symbol = "eUSDC";
    ...
    function mintAndDepositPool(uint64 amount, address token, address pool, address from) public onlyCallerContract {
        _mint(amount, to);
        ...
        darkPool.depositDP(tokenId, amount);
    }

    function burnAndWithdrawPool(uint64 amount, address token, address pool, address to) public onlyCallerContract {
        _burn(amount, to);
        ...
        IDarkPool darkPool = IDarkPool(pool);
        darkPool.withdrawDP(tokenId, amount);
    }
...
/**
 * @title MintableEncryptedERC20 that mirrors the ERC20 standard with encrypted balances.
 * @notice check mintAndDepositPool function to mint tokens and deposit them to the pool
 */
abstract contract MintableEncryptedERC20 is BridgeContract, EncryptedErrors {
    enum ErrorCodes {
        NO_ERROR,
    ...
    constructor() EncryptedErrors(uint8(type(ErrorCodes).max)) {
        // Base Sepolia and iexRouter router from inco
        BridgeContract.initialize(84532, address(this), 0x015b8be6946ee593Ee2230E56221Db9cEE22aC20);
    }
